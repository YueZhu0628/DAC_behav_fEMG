...
rescore_items <- function(data, items){
  data %>%
    mutate(across(all_of(matches(items)), ~ recode(., "0" = 1, "1" = 2, "2" = 3, "3" = 4, "4" = 5)))
}
```
...
reverse_score_items_bai <- function(data, items){
  data %>%
    mutate(across(all_of(matches(items)), ~ recode(., "1" = 4, "2" = 3, "3" = 2, "4" = 1)))
}

reverse_score_items_nfc <- function(data, items){
  data %>%
    mutate(across(all_of(matches(items)), ~ recode(., "1" = 5, "2" = 4, "3" = 3, "4" = 2, "5" = 1)))
}
...
combined_dataframe_reversed <- 
  combined_dataframe_rescored %>% 
  reverse_score_items_bai(bai_items) %>%  ## BIS_BAS
  reverse_score_items_nfc(nfc_items) %>%  ## NFC
  reverse_score_items_bfi(bfi_items) %>%  ## BFI
  reverse_score_items_upps(upps_items)  ## UPPS-P
