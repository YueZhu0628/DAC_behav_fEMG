...
var PID_ID = {
  ...
  questions: [
    ...
    {prompt: "Please enter your SONA ID", name:'SONA', required: true},
    ],
  on_finish: function(data) {
    PID  = JSON.parse(data.responses)['PID'];
    SONA = JSON.parse(data.responses)['SONA'];
    jsPsych.data.addProperties({
      'PID':   JSON.parse(data.responses)['PID'],
      'SONA':  JSON.parse(data.responses)['SONA'],
    });
...
var end_screen = {
  ...
  stimulus: `Thank you for your participation! You are now finished the entire study. <br> \
             We will grant you 1.5 SONA credit shortly. <br> \
...
var BFI = {
  ...
  questions: [
    ...
    {prompt: "Can be moody.", name:'BAI29', required: true, labels: likert_scale},
...
jsPsych.init({
  timeline: timeline,
  on_finish: function() {
    jsPsych.data.get().localSave('csv', `${SONA}_survey.csv`);
  }
});