...
df_Learning %>% 
  ...
  group_by(PID, TaskDemand) %>%
  summarise(Task_ACC = mean(Task_ACC, na.rm=TRUE), .groups = "drop") %>%
  mutate(PID = factor(PID)) %>%
  ggplot(data=., aes(x=PID, y = Task_ACC,
            color = TaskDemand, fill = TaskDemand)) +
  geom_bar(stat = "identity", position = position_dodge(0.5)) +
  scale_color_lancet()+
  scale_fill_lancet(alpha = 0.5)+
  labs(title = "Task ACC", x = "PID", y = "Task ACC") + 
  theme_bw() + 
  theme(plot.title = element_text(hjust = 0.5))
```
...
```{r}
df_Learning %>% 
  ggboxplot(data=., x="PID", y = "Task_ACC", 
          color = "TaskDemand", 
          width = 0.7, dodge = 0) + 
  scale_color_lancet()+
  labs(title = "Task ACC", x = "PID", y = "Task ACC", color = "Task Demand") + 
  theme_bw() + 
  theme(plot.title = element_text(hjust = 0.5))
```
...
df_Learning %>% 
  # group_by(PID) %>% 
  # summarise(Task_A_measure = mean(Task_A_measure, na.rm=TRUE), .groups = "drop") %>%
  # mutate(PID = factor(PID)) %>% 
  # ggplot(data=., aes(x=PID, y = Task_A_measure)) + 
  # geom_bar(stat = "identity", position = position_dodge(0.15)) +
  group_by(PID, TaskDemand) %>%
  summarise(Task_A_measure = mean(Task_A_measure, na.rm=TRUE), .groups = "drop") %>%
  mutate(PID = factor(PID)) %>%
  ...
  scale_color_lancet()+
  scale_fill_lancet(alpha = 0.5)+
  labs(title = "Task_A_measure", x = "PID", y = "Task_A_measure") + 
  theme_bw() + 
  theme(plot.title = element_text(hjust = 0.5))
```
...
df_True_Choice %>% 
  ...
  scale_color_lancet()+
  scale_fill_lancet(alpha = 0.5)+
  labs(title = "Task RT", x = "PID", y = "Task RT (s)") + 
  theme_bw() + 
  theme(plot.title = element_text(hjust = 0.5))
```
...
df_False_Choice %>% 
  ...
  scale_color_lancet()+
  scale_fill_lancet(alpha = 0.5)+
  labs(title = "Task RT", x = "PID", y = "Trial RT (s)") + 
  theme_bw() + 
  theme(plot.title = element_text(hjust = 0.5))
```
...
df_False_Choice %>% 
  ggboxplot(data=., x="PID", y = "Task_RT", 
          ...
          width = 0.7, dodge = 0) + 
  scale_color_lancet()+
  labs(title = "Task RT", x = "PID", y = "Task RT (s)", color = "Choice Demand") + 
  theme_bw() + 
  theme(plot.title = element_text(hjust = 0.5))
```
...
df_False_Choice %>% 
  ggboxplot(data=., x="PID", y = "Task_ACC", 
          ...
          width = 0.7, dodge = 0) + 
  scale_color_lancet()+
  labs(title = "Task ACC", x = "PID", y = "Task ACC", color = "Choice Demand") + 
  theme_bw() + 
  theme(plot.title = element_text(hjust = 0.5))
```
...
(paired t-test. compare the RT, ACC, A measure in 1-back and 3-back trials)
### Learning Phase
#### RT
paired by demands within participant
```{r}
pw_rt_learning <- 
  df_Learning %>% 
  group_by(PID) %>%
  t_test(Task_RT_log_z ~ TrialDemandLearning_code, paired = TRUE, detailed = TRUE) %>% 
  add_significance() %>%
  add_xy_position(x = "TrialDemandLearning_code")
# table(pw_rt_learning$p.signif)
```
