...
```{r}
# add the excluded PIDs due performance outliers or other problems
# excludedPIDs <-
...
```{r}
perf_bar_plot <- function(df_clean, indicator, gb, demands=TRUE){
  if(demands == FALSE){
    df <- 
      df_clean %>%
      filter(Phase %in% c("LEARNING","TrueCHOICE","FalseCHOICE")) %>% 
      group_by(PID) %>%
      summarise(rt_mean = mean(!!sym(indicator), na.rm=TRUE), .groups = "drop")
  }else{
    df <-
      df_clean %>%
      filter(Phase %in% c("LEARNING","TrueCHOICE","FalseCHOICE")) %>%
...
Histogram Plots
```{r}
perf_hist_plot(df_clean, "Task_RT")
perf_hist_plot(df_clean, "Task_ACC")
perf_hist_plot(df_clean, "Task_A_measure")
```

Bar Plots
```{r}
perf_bar_plot(df_clean,"Task_RT")
perf_bar_plot(df_clean,"Task_ACC")
perf_bar_plot(df_clean,"Task_A_measure")
```

Box Plots
```{r}
perf_box_plot(df_clean, "Task_RT", "TaskDemand")
perf_box_plot(df_clean, "Task_ACC", "TaskDemand")
perf_box_plot(df_clean, "Task_A_measure", "TaskDemand")
```
...
Bar Plots
```{r}
perf_bar_plot(df_Learning, "Task_RT")
perf_bar_plot(df_Learning, "Task_ACC")
perf_bar_plot(df_Learning, "Task_A_measure")
```

Box Plots
```{r}
perf_box_plot(df_Learning, "Task_RT", "TaskDemand")
perf_box_plot(df_Learning, "Task_ACC", "TaskDemand")
perf_box_plot(df_Learning, "Task_A_measure", "TaskDemand")
```
...
Bar Plots
```{r}
perf_bar_plot(df_True_Choice, "Task_RT")
perf_bar_plot(df_True_Choice, "Task_ACC")
perf_bar_plot(df_True_Choice, "Task_A_measure")
```

Box Plots
```{r}
perf_box_plot(df_True_Choice, "Task_RT", "TaskDemand")
perf_box_plot(df_True_Choice, "Task_ACC", "TaskDemand")
perf_box_plot(df_True_Choice, "Task_A_measure", "TaskDemand")
```
...
Bar Plots
```{r}
perf_bar_plot(df_False_Choice, "Task_RT", "ChoiceDemand")
perf_bar_plot(df_False_Choice, "Task_ACC", "ChoiceDemand")
perf_bar_plot(df_False_Choice, "Task_A_measure", "ChoiceDemand")
```
...
```{r}
# PID 10, 41, 45 rated 3-back as harder
```
...
df_Learning %>% 
  ...
  geom_histogram(position = position_dodge(width = 0.4), bins = 10, alpha = 0.7)+
  # scale_x_continuous(breaks = seq(-5, 5)) +
  scale_color_lancet() + 
  scale_fill_lancet() + 
  labs(title = "fEMG response Means of participants", x = "fEMG values (z-scored)", y = "Counts", fill = "Task Demand") + 
...
df_Learning %>% 
  group_by(PID, TaskDemand) %>% 
  summarise(cor_z = mean(cue_e_on_cor_zvalues, na.rm = TRUE), .groups = "drop") %>% 
  ggplot(aes(x = cor_z, color  = TaskDemand, fill = TaskDemand)) +
  geom_histogram(position = position_dodge(width = 0.5), bins = 10, alpha = 0.7)+
  # scale_x_continuous(breaks = seq(-5, 5)) +
  scale_color_lancet() + 
...
df_zyg_means_de <- 
  df_Learning %>% 
  group_by(PID, TrialDemandLearning) %>% 
  summarise(ISI = mean(ISI, na.rm=TRUE),
    zyg_z = mean(cue_i_on_zyg_zvalues, na.rm = TRUE), 
            .groups = "drop") %>% 
  group_by(PID) %>% 
  mutate(zyg_z_diff = 
           zyg_z[TrialDemandLearning == "Easy"] - zyg_z[TrialDemandLearning == "Hard"]) %>%
  ungroup()

t_zyg <- 
  df_zyg_means_de %>%
  t_test(zyg_z ~ TrialDemandLearning,  detailed = TRUE) %>% 
  add_xy_position(x = "TrialDemandLearning") %>% 
  add_significance()


df_zyg_means_de %>%
  ggplot(aes(x = TrialDemandLearning, y = zyg_z, color = TrialDemandLearning)) + 
  ...
  stat_pvalue_manual(t_zyg, y.position = 1, label = "p = {p}{p.signif}") +
  labs(title = "zyg t-test", x = "Task Demand", y = "zyg_z", 
       color = "Task Demand") + 
  theme_bw() + 
  theme(plot.title = element_text(hjust = 0.5))
```
...
df_zyg_means_de <- 
  ...
  group_by(PID, TrialDemandLearning) %>% 
  summarise(ISI = mean(ISI, na.rm=TRUE),
    zyg_z = mean(cue_e_on_zyg_zvalues, na.rm = TRUE), 
            .groups = "drop") %>% 
  group_by(PID) %>% 
  mutate(zyg_z_diff = 
           zyg_z[TrialDemandLearning == "Easy"] - zyg_z[TrialDemandLearning == "Hard"]) %>%
...
df_cor_means_de <- 
  ...
  group_by(PID, TrialDemandLearning) %>% 
  summarise(ISI = mean(ISI, na.rm=TRUE),
    cor_z = mean(cue_i_on_cor_zvalues, na.rm = TRUE), 
            .groups = "drop") %>% 
  group_by(PID) %>% 
  mutate(cor_z_diff = 
           cor_z[TrialDemandLearning == "Easy"] - cor_z[TrialDemandLearning == "Hard"]) %>%
  ungroup()

t_cor <- 
  df_cor_means_de %>%
  t_test(cor_z ~ TrialDemandLearning,  detailed = TRUE) %>% 
  add_xy_position(x = "TrialDemandLearning") %>% 
  add_significance()


df_cor_means_de %>%
  ggplot(aes(x = TrialDemandLearning, y = cor_z, color = TrialDemandLearning)) + 
  geom_boxplot(position = "identity", alpha = 0.7, width = 0.5, outlier.color = "black") +
  stat_boxplot(geom = "errorbar", width = 0.15) +
  geom_jitter(width = 0.15) + 
  scale_color_lancet() + 
  stat_summary(fun = mean, geom = "point", shape = 17, color = "black", size = 2) +
  stat_pvalue_manual(t_cor, y.position = 1, label = "p = {p}{p.signif}") +
  labs(title = "cor t-test", x = "Task Demand", y = "cor_z", 
       color = "Task Demand") + 
  theme_bw() + 
  theme(plot.title = element_text(hjust = 0.5))
```
...
df_cor_means_de <- 
  ...
  group_by(PID, TrialDemandLearning) %>% 
  summarise(ISI = mean(ISI, na.rm=TRUE),
    cor_z = mean(cue_e_on_cor_zvalues, na.rm = TRUE), 
            .groups = "drop") %>% 
  group_by(PID) %>% 
  mutate(cor_z_diff = 
           cor_z[TrialDemandLearning == "Easy"] - cor_z[TrialDemandLearning == "Hard"]) %>%
...
```{r}
# function of plot means of each 10 trials
```
...
result_list <- lapply(names(trial_sections), function(section_name) {
  ...
  result <- calculate_section_means(df_Learning, "cue_i_on_cor_zvalues", section)
...
df_femg_means_demands_tsecs <- bind_rows(result_list)

df_femg_grandmeans_demands_tsecs <-
...
df_zyg_means_de <- 
  ...
  summarise(ISI = mean(ISI, na.rm=TRUE),
    zyg_z = mean(cue_e_true_zyg_zvalues, na.rm = TRUE), 
            .groups = "drop") %>% 
  # group_by(PID) %>% 
  # mutate(zyg_z_diff = 
  #          zyg_z[ChoiceDemand == "Easy"] - zyg_z[ChoiceDemand == "Hard"]) %>%
  ungroup()

t_zyg <- 
  df_zyg_means_de %>%
  t_test(zyg_z ~ ChoiceDemand,  detailed = TRUE) %>% 
  add_xy_position(x = "ChoiceDemand") %>% 
  add_significance()


df_zyg_means_de %>%
  ggplot(aes(x = ChoiceDemand, y = zyg_z, color = ChoiceDemand)) + 
  geom_boxplot(position = "identity", alpha = 0.7, width = 0.5, outlier.color = "black") +
  stat_boxplot(geom = "errorbar", width = 0.15) +
  geom_jitter(width = 0.15) + 
  scale_color_lancet() + 
  stat_summary(fun = mean, geom = "point", shape = 17, color = "black", size = 2) +
  stat_pvalue_manual(t_zyg, y.position = 1, label = "p = {p}{p.signif}") +
  labs(title = "zyg t-test", x = "Choice Demand", y = "zyg_z", 
       color = "Choice Demand") + 
  theme_bw() + 
  theme(plot.title = element_text(hjust = 0.5))
```
...
df_cor_means_de <- 
  ...
  group_by(PID, ChoiceDemand) %>% 
  summarise(ISI = mean(ISI, na.rm=TRUE),
    cor_z = mean(cue_e_true_cor_zvalues, na.rm = TRUE), 
            .groups = "drop") %>% 
  # group_by(PID) %>% 
  # mutate(zyg_z_diff = 
  #          zyg_z[ChoiceDemand == "Easy"] - zyg_z[ChoiceDemand == "Hard"]) %>%
...
t_cor <- 
  df_cor_means_de %>%
  t_test(cor_z ~ ChoiceDemand,  detailed = TRUE) %>% 
  add_xy_position(x = "ChoiceDemand") %>% 
  add_significance()


df_cor_means_de %>%
  ggplot(aes(x = ChoiceDemand, y = cor_z, color = ChoiceDemand)) + 
  geom_boxplot(position = "identity", alpha = 0.7, width = 0.5, outlier.color = "black") +
  stat_boxplot(geom = "errorbar", width = 0.15) +
  geom_jitter(width = 0.15) + 
  scale_color_lancet() + 
  stat_summary(fun = mean, geom = "point", shape = 17, color = "black", size = 2) +
  stat_pvalue_manual(t_cor, y.position = 1, label = "p = {p}{p.signif}") +
  labs(title = "cor t-test", x = "Choice Demand", y = "cor_z", 
       color = "Choice Demand") + 
  theme_bw() + 
  theme(plot.title = element_text(hjust = 0.5))
```
...
df_zyg_means_de <- 
  ...
  group_by(PID, ChoiceDemand) %>% 
  summarise(ISI = mean(ISI, na.rm=TRUE),
    zyg_z = mean(cue_e_false_zyg_zvalues, na.rm = TRUE), 
            .groups = "drop") %>% 
  # group_by(PID) %>% 
  # mutate(zyg_z_diff = 
  #          zyg_z[ChoiceDemand == "Easy"] - zyg_z[ChoiceDemand == "Hard"]) %>%
...
df_cor_means_de <- 
  ...
  group_by(PID, ChoiceDemand) %>% 
  summarise(ISI = mean(ISI, na.rm=TRUE),
    cor_z = mean(cue_e_false_cor_zvalues, na.rm = TRUE), 
            .groups = "drop") %>% 
  # group_by(PID) %>% 
  # mutate(zyg_z_diff = 
  #          zyg_z[ChoiceDemand == "Easy"] - zyg_z[ChoiceDemand == "Hard"]) %>%
